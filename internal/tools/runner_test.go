...
func TestRunnerRejectsUnsafeArgument(t *testing.T) {
	r := NewRunner(nil)
	_, err := r.Run(context.Background(), ToolSpec{Name: "ls", Args: []string{"bad;arg"}})
	if err == nil {
...
func TestRunnerAllowlistsUname(t *testing.T) {
	r := NewRunner(nil)
	res, err := r.Run(context.Background(), ToolSpec{Name: "uname", Args: []string{"-s"}, Timeout: time.Second})
	if err != nil {
