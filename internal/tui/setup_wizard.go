...
func NewSetupWizardState(configPath string, existing *config.Config) SetupWizardState {
	...
	if endpoint := strings.TrimSpace(existing.Provider.Endpoint); endpoint != "" {
...
func (s SetupWizardState) IsLocalProvider() bool {
	return strings.EqualFold(s.Provider, string(config.ProviderLocal)) || strings.EqualFold(s.Provider, string(config.ProviderOllama))
}
...
func (s SetupWizardState) PromptLabel() string {
	...
	case setupStepProvider:
		return "Provider (openai|anthropic|gemini|ollama|local)"
	case setupStepEndpoint:
...
func (s *SetupWizardState) Submit(raw string) SetupWizardSubmitResult {
	...
	case setupStepEndpoint:
		if input == "" {
			s.ErrorMessage = "Endpoint cannot be empty."
			return SetupWizardSubmitResult{Output: s.ErrorMessage}
		}
		s.Endpoint = input
		s.Step = setupStepModel
		return SetupWizardSubmitResult{
			Output:             fmt.Sprintf("Endpoint set to %s. Discovering available Ollama models...", s.Endpoint),
			NeedModelDiscovery: true,
			DiscoveryEndpoint:  s.Endpoint,
		}
	case setupStepModel:
		if input == "" {
...
func (s SetupWizardState) BuildConfig() (config.Config, error) {
	...
	if s.IsLocalProvider() {
		cfg.Provider.Endpoint = strings.TrimSpace(s.Endpoint)
	} else {
