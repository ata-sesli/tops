...
func parseModelConfig(raw string, payload string) ParseResult {
	...
	usage := "Usage: /model config set <context|max_length|system_prompt|intelligence_mode|think|temperature|top_k|top_p|min_p|repeat_penalty|think_budget_tokens> <value>"
	if rest == "" {
	...
	switch field {
	case "context", "max_length", "system_prompt", "intelligence_mode", "intelligence", "think", "temperature", "top_k", "top_p", "min_p", "repeat_penalty", "think_budget_tokens":
		if field == "intelligence" {
			field = "intelligence_mode"
		}
