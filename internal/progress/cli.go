...
func (r *CLIReporter) ActionCompleted(stepID string, commandLine string, actionClass string, exitCode int, errText string) {
	if strings.TrimSpace(errText) != "" {
		r.writeWorkflowLine(fmt.Sprintf("action completed[%s] (%s) exit=%d error=%s", strings.TrimSpace(stepID), strings.TrimSpace(actionClass), exitCode, strings.TrimSpace(errText)))
		return
	}
	r.writeWorkflowLine(fmt.Sprintf("action completed[%s] (%s) exit=%d", strings.TrimSpace(stepID), strings.TrimSpace(actionClass), exitCode))
}
