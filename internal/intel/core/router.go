...
func (r Router) Dispatch(ctx context.Context, mode model.Mode, input string, cfg config.Config, askProfile model.AskResponseProfile, intelligenceMode model.IntelligenceMode, platform model.PlatformContext) (any, error) {
	request, err := normalizeRequest(mode, input, cfg, askProfile, intelligenceMode, platform)
	if err != nil {
		return nil, err
	}
	switch request.Mode {
...
func normalizeRequest(mode model.Mode, input string, cfg config.Config, askProfile model.AskResponseProfile, intelligenceMode model.IntelligenceMode, platform model.PlatformContext) (model.CoreRequest, error) {
	...
	return model.CoreRequest{
		...
		Shell:                   shell,
		OS:                      runtime.GOOS,
		PlatformContext:         model.NormalizePlatformContext(platform),
		IntelligenceMode:        model.NormalizeIntelligenceMode(string(intelligenceMode)),
